...
| 二 | VLAN与园区二层/三层架构 | HQ VLAN10/20/30、Po1静态EtherChannel、Core SVI；Branch VLAN40/50、Trunk/ROAS；IPv4 DHCP | [二层](FINAL_CONFIGURATION.md#layer2)、[IPv4](FINAL_CONFIGURATION.md#ipv4)；N1/N3/N4、图47 |
| 三 | 企业出口访问控制与公网服务发布 | Core SVI ACL、HQ/Branch PAT、协议级ACL、TCP80映射、Internet DNS/HTTP | [安全](FINAL_CONFIGURATION.md#security)、[服务](FINAL_CONFIGURATION.md#services)；N2/N7–N11、图48–49 |
| 四 | 企业路由协议与跨域互联 | 生产OSPF/eBGP；VLAN100双DROTHER；R-COURSE/R-TEST隔离重分发 | [路由](FINAL_CONFIGURATION.md#routing)；N5/N6、图50–52 |
| 五 | 网络安全增强与IPv6 Overlay | Access Fa0/1与Fa0/3 sticky/max1/restrict；R-HQ/R-BRANCH IPv6-over-IPv4 Tunnel | [端口安全](FINAL_CONFIGURATION.md#security)、[Overlay](FINAL_CONFIGURATION.md#tunnel)；N15/N13、图53–54 |
...
| E2-07 | EtherChannel成员聚合 | Core Gi1/0/1–2、Access Gi0/1–2 | 最终使用`channel-group 1 mode on`静态聚合；成员(P)、Po1(SU)及Protocol列`-`为验证依据，不再宣称LACP协商 |
...
| E3-08 | PAT/Overload | R-HQ OFFICE `/24`与R-BRANCH OFFICE `/26` | 多个内部会话复用出口地址；Branch以`BR-NAT-INSIDE`限定到`192.0.2.0/24`，TCP/80翻译和统计为图48证据；IoT未获通用PAT |
...
| E4-02 | OSPF Area 0 | Transit `10.255.0.0/29`及Core HQ网段 | 单骨干区域；VLAN100上Core为DR，R-HQ与R-COURSE均为FULL/DROTHER，图50交叉验证邻居和接口状态 |
...
| E4-05 | OSPF邻居与动态路由学习 | Core/R-HQ/R-COURSE | 生产路由保持；课程增量用priority255/0/0固定本次广播网段选举，形成两个FULL/DROTHER |
...
| E4-11 | OSPF+BGP融合/隔离重分发 | 生产R-HQ及课程R-COURSE/R-TEST | 生产WAN仍不做广泛重分发；课程域将测试前缀10.54.54.0/24以O E2/Type-5注入OSPF1，并向R-TEST发布O*E2默认路由 |
...
| ID | 课程技术 | 在当前项目用在哪里 | 作用、配置要点与验证 |
|---|---|---|---|
| E5-01 | Port Security | SW-ACCESS Fa0/1→OFFICE-PC、Fa0/3→ADMIN-PC | VLAN10与VLAN30各选一个接入口启用安全，形成双端口对照；不是对每个交换端口都启用 |
| E5-02 | Sticky MAC | Fa0/1与Fa0/3 | 自动学习合法MAC；当前SecureSticky分别为00E0.F9B0.77EE和00D0.97E2.907A，保存入startup-config需保存重开核验 |
| E5-03 | Maximum MAC | Fa0/1、Fa0/3 maximum1 | 每个受控端口只允许一个安全MAC；不是整个VLAN只允许一台终端 |
| E5-04 | MAC Binding/端口绑定 | OFFICE↔Fa0/1/VLAN10；ADMIN↔Fa0/3/VLAN30 | sticky实现绑定；`show port-security address`与两个接口详情交叉核对 |
| E5-05 | Violation Mode选择 | Fa0/1、Fa0/3 | 两端均选择restrict；课程图54在Fa0/3临时接入非法PC后恢复ADMIN-PC |
| E5-06 | Restrict | Fa0/3非法MAC流量 | 丢弃非法流量并累计5次违规，端口保持Secure-up；恢复合法ADMIN-PC后ping恢复 |
