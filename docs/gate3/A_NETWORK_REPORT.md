...
> **2026-09-17 最终状态更正**：本报告保留 G3 分层实施的历史输出与真实截图；G3-A-03c 的私网 HTTP 页面仅证明当时阶段。用户最新实测在 static TCP/80 映射共存后，Branch 私网直连 http://192.168.30.10 FAIL，而 http://203.0.113.1→192.168.30.10:80 PASS；私网 ping PASS。后者为最终业务入口，前者不得继续作为最终 N7 PASS。G4 A 已实测（用户确认），图后补；当前包哈希和待补项见 docs/gate4/A_NETWORK_REPORT.md。本报告末尾旧“后续未实施”仅是 G3 当时记录。
