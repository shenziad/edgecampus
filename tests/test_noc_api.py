...
class NocApiTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_network_failure_management_and_policy_overlay(self):
        ...
        _, down = await self.request("/api/simulation/network", {})
        self.assertEqual((down["ospf"], down["bgp"], down["ipv6_tunnel"], down["branch_status"]), ("FULL", "DOWN", "DOWN", "OFFLINE"))
        _, unavailable = await self.request("/api/branch/check", {"device": "R-BRANCH"})
        self.assertEqual(unavailable["management"], "UNAVAILABLE")
        _, recovered = await self.request("/api/simulation/network/restore", {})
        self.assertEqual(recovered["bgp"], "ESTABLISHED")
