...
# ============================================================
# EdgeCampus Gate 4 - EDGE-SBC-01
# Failure Recovery + Local Autonomy + State Sync
# ============================================================
...
# Default Policy
#
# These values are startup defaults only.
# Once a valid Cloud Policy is applied, the runtime policy
# remains in memory during a Cloud outage.
# ============================================================
...
def timestamp():
    ...
    # keep a valid ISO-8601 timestamp without relying on
...
def on_connection_change(state):
    ...
    if connection_ready:
        ...
        # Every new connection starts a fresh Cloud bootstrap.
        hello_sent = False
        state_sync_sent = False

        # Force one fresh FAN status after connection recovery.
        last_reported_fan_state = "UNKNOWN"

        if connection_count == 1:
            print(">>> CLOUD CONNECTED <<<")
        else:
            print(">>> CLOUD RECONNECTED <<<")
        ...
        print(
            "Local control remains active with Policy v"
            + str(POLICY_VERSION)
            + " / "
            + MODE
            + " / TH="
...
def on_receive(data):
    # Never block inside callback.
    # Queue Cloud messages for main loop processing.
...
def send_state_sync():
    # Restore the real Edge state after Backend restart.
    #
    # state_sync contains:
    # - temperature
    # - fan state
    # - complete last valid runtime Policy
    ...
    message = (
        envelope("state_sync")
        + ',"edge_id":"' + EDGE_ID + '"'
        + ',"temperature_c":' + str(value)
        + ',"fan_state":"' + fan_state + '"'
        + ',"policy":{'
        + '"policy_id":"' + POLICY_ID + '"'
        + ',"version":' + str(POLICY_VERSION)
        + ',"mode":"' + MODE + '"'
        + ',"threshold_c":' + str(THRESHOLD_C)
        + ',"hysteresis_c":' + str(HYSTERESIS_C)
        + '}}'
    )
...
def process_policy(raw):
    ...
    message_protocol = json_value(
        raw,
        "protocol_version"
    )

    policy_id = json_value(
        raw,
        "policy_id"
    )

    version_text = json_value(
        raw,
        "version"
    )
    ...
    threshold_text = json_value(
        raw,
        "threshold_c"
    )

    hysteresis_text = json_value(
        raw,
        "hysteresis_c"
    )
    ...
    # Re-evaluate current temperature immediately when
    # switching/updating AUTO policy.
...
def process_command(raw):
    ...
    command_id = json_value(
        raw,
        "command_id"
    )

    device_id = json_value(
        raw,
        "device_id"
    )
...
def main():
    ...
    # IMPORTANT:
    # connection_ready is written inside main(),
    # so it MUST be declared global.
    global connection_ready
    ...
    print("")
    print("====================================================")
    print(" EdgeCampus Gate 4 - EDGE-SBC-01")
    print(" Failure Recovery + Local Autonomy + State Sync")
    print("====================================================")
    ...
    print(
        "Reconnect      : every "
        + str(RECONNECT_INTERVAL_MS)
        + " ms"
    )

    print("----------------------------------------------------")
    print("Local Loop never depends on Cloud availability.")
    print("Last valid runtime Policy survives Cloud outage.")
    print("Reconnect does not block Local Loop.")
    print("Reconnect sends hello + real state_sync.")
    print("====================================================")
    print("")
    ...
    try:
        ...
        print(
            "Initial Cloud connection failed."
        )
    ...
    while True:
        ...
        while len(pending_messages) > 0:
            ...
            process_cloud_message(
                raw_message
            )

        # ====================================================
        # C. GATE 4 NON-BLOCKING RECONNECT
        # ====================================================
        ...
        if not client.connected():
            ...
            # Main-loop fallback:
            # keep state correct even if PT callback timing
            # is slightly delayed.
        ...
        if (
            ...
            # -----------------------------------------------
            # 2. Fresh connection -> STATE_SYNC
            #
            # Wait until a real temperature exists.
            # -----------------------------------------------
